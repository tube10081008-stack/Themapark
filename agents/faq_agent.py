...
# 사고 관련 동사는 과거형·미래형·어간을 함께 등록할 것.
#
# 세 묶음으로 나눈 이유: 문의(faq)는 전부 사람에게 넘겨야 하지만, 공개 리뷰에서는
# 라우팅 계열('사장', '책임자' 등)이 오탐을 낸다. "사장님이 친절하셨어요" 같은
# 호평까지 차단되면 분류 통계에서 통째로 빠진다. review_agent 는 앞의 두 묶음만 쓴다.
...
# 문의 라우팅용 — 공개 리뷰에는 적용하지 않는다 (호평 오탐이 잦음)
ROUTING_KEYWORDS = [
    "민원", "불만", "항의", "책임자", "사장", "군청", "취소해", "신고할",
]

ESCALATE_KEYWORDS = SAFETY_KEYWORDS + DISPUTE_KEYWORDS + ROUTING_KEYWORDS
